        ...
        {payments.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {payments.map((payment) => (
              <div
                key={payment.id}
                className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-red-400/70 p-6 hover:shadow-2xl transition-all duration-300 flex flex-col"
              >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm">
                      ...
                      <div>
                        ...
                        <span className="font-mono text-red-700">{payment.reference || 'N/A'}</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="border-t border-gray-100/70 pt-3 mt-3 space-y-2">
                  {payment.passCode && (
                    <div>
                      ...
                      <span className="inline-block font-mono bg-gradient-to-r from-red-100 to-pink-50 text-red-700 px-3 py-1 rounded-lg text-sm">{payment.passCode}</span>
                    </div>
                  )}
                  {payment.ticketRoute && (
                    <div>
                      <p className="text-xs text-gray-500 mb-1">Ticket Route</p>
                      <span className="inline-block font-mono bg-gradient-to-r from-blue-100 to-cyan-50 text-blue-700 px-3 py-1 rounded-lg text-sm">{payment.ticketRoute}</span>
                    </div>
                  )}
                  {payment.proofUrl && (
                    <div>
                      ...
                      <a
                        ...
                        className="text-red-700 font-medium hover:underline break-all"
                      >
                        View Proof
                      </a>
                    </div>
