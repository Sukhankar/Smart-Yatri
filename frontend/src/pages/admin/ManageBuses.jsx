...
export default function ManageBuses() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 to-pink-100 flex flex-col md:flex-row">
        <Sidebar role="admin" />
        <main className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-red-600 mx-auto mb-4"></div>
            <p className="text-base font-medium text-red-800">Loading bus data...</p>
          </div>
        </main>
      </div>
    );
      ...
      <main className="flex-1 flex flex-col px-2 sm:px-4 md:px-8 py-4">
        ...
        <div className="w-full flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center mb-5">
          ...
          <button
            ...
            className="inline-flex items-center justify-center bg-gradient-to-br from-red-600 to-pink-600 text-white font-semibold px-6 py-2 rounded-xl shadow-md hover:shadow-lg hover:from-red-700 hover:to-pink-700 transition-all duration-200 focus:outline-none focus:ring focus:ring-red-300"
          >
          ...
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-red-100">
            <table className="w-full text-sm">
              <thead className="bg-red-50 border-b border-red-100">
                <tr>
                  <th className="px-6 py-3 text-left font-semibold text-red-900 uppercase tracking-wider">Number Plate</th>
                  <th className="px-6 py-3 text-left font-semibold text-red-900 uppercase tracking-wider">Route</th>
                  <th className="px-6 py-3 text-left font-semibold text-red-900 uppercase tracking-wider">Conductor</th>
                  <th className="px-6 py-3 text-left font-semibold text-red-900 uppercase tracking-wider">Seats</th>
                  <th className="px-6 py-3 text-left font-semibold text-red-900 uppercase tracking-wider">Status</th>
                  <th className="px-6 py-3 text-left font-semibold text-red-900 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody>
                {buses.length === 0 && (
                  <tr>
                    <td colSpan={6} className="text-center py-10 text-red-500">No buses found.</td>
                  </tr>
                )}
                {buses.map((bus) => (
                  <tr
        ...
        <div className="md:hidden w-full flex flex-col gap-4">
          {buses.length === 0 ? (
            <div className="w-full text-center py-10 bg-white rounded-2xl shadow-md border border-red-100 text-red-500">
              No buses found.
            </div>
          ) : (
            buses.map((bus) => (
              <div
                key={bus.id}
                className="bg-white rounded-xl shadow-md border border-red-100 px-4 py-5 flex flex-col gap-1"
              >
                <div className="flex justify-between items-center mb-2">
                  <div className="flex-1">
                    <span className="block text-lg font-black text-gray-800">{bus.numberPlate}</span>
                    <span className="block text-xs text-red-500">{bus.route?.name || <span className="italic text-gray-400">Not assigned</span>}</span>
                  </div>
                  <span
        ...
        {showModal && (
          <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40">
            <div className="relative w-full max-w-lg mx-2 sm:mx-auto bg-white rounded-2xl shadow-lg border border-red-100 p-6 overflow-y-auto max-h-[90vh]">
              <button
                onClick={() => setShowModal(false)}
                className="absolute top-4 right-4 text-red-400 hover:text-red-500 hover:bg-red-50 rounded-full p-2 transition"
                aria-label="Close"
                tabIndex={0}
              >
              ...
              <h2 className="text-xl font-black mb-6 text-red-800">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 border border-red-200 rounded focus:ring-2 focus:ring-red-200"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-700 mb-1">
                ...
                <div className="flex justify-end gap-3 pt-5">
                  ...
                  <button
                    type="submit"
                    className="px-6 py-2 bg-gradient-to-br from-red-600 to-pink-600 rounded-lg text-white font-semiblod shadow hover:from-red-700 hover:to-pink-700 transition"
                  >
